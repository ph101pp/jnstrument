...
function jnstrumentVisitor(babel) {
  ...
  function createEnterEvent(ids) {
    ...
    return t.variableDeclaration('const', [t.variableDeclarator(ids.currentScopeId, t.callExpression(t.memberExpression(ids.event, t.identifier('i')), params))]);
  }

  function createExitEvent(ids, returns) {
    const params = [returns || t.identifier('undefined'), ids.functionId, ids.currentScopeId];
  ...
  const ProgramVisitor = {
    ArrowFunctionExpression(path) {
      ...
      path.replaceWith(t.functionExpression(t.identifier('f' + Math.random().toString(36).substr(2, 8)), path.node.params, path.node.body));
    },
    Function(path) {
      if (path.node.id === null) {
        path.node.id = t.identifier('f' + Math.random().toString(36).substr(2, 8));
      }
      const name = path.node.id && path.node.id.name || path.node.key && path.node.key.name;
