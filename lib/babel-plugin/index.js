...
function jnstrumentVisitor(babel) {
  ...
  function createJnstrumentation(id, name, ...otherParams) {
    return t.callExpression(t.memberExpression(id, t.identifier(name)), [...otherParams]);
  }
  ...
  const ProgramVisitor = {
    Function(path) {
      ...
      body.unshiftContainer('body', t.expressionStatement(createJnstrumentation(this.eventIdentifier, 'enter')));
      body.pushContainer('body', t.expressionStatement(createJnstrumentation(this.eventIdentifier, 'exit')));
    },
    ClassBody(path) {
    ...
    ReturnStatement(path) {
      ...
      path.node.argument = createJnstrumentation(this.eventIdentifier, 'exit', path.node.argument);
    }
  };
  ...
  return {
    ...
    visitor: {
      Program(path) {
        ...
        path.node.body.unshift(t.expressionStatement(createJnstrumentation(identifier, 'enter')));
        path.node.body.unshift(requireStatement);
        path.node.body.push(t.expressionStatement(createJnstrumentation(identifier, 'exit')));
