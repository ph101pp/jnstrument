...
function jnstrumentVisitor(babel) {
  ...
  const ProgramVisitor = {
    Function(path) {
      ...
      body.unshiftContainer('body', t.expressionStatement(createJnstrumentation(this.eventIdentifier, 'i')));
      
      body.find((node)=>node.type === 'returnStatement') && body.pushContainer('body', t.expressionStatement(createJnstrumentation(this.eventIdentifier, 'o')));
    },
    ClassBody(path) {
    ...
    ReturnStatement(path) {
      ...
      path.node.argument = createJnstrumentation(this.eventIdentifier, 'o', path.node.argument);
    }
  };
  ...
  return {
    ...
    visitor: {
      Program(path) {
        ...
        path.node.body.unshift(t.expressionStatement(createJnstrumentation(identifier, 'i')));
        path.node.body.unshift(requireStatement);
        path.node.body.push(t.expressionStatement(createJnstrumentation(identifier, 'o')));
